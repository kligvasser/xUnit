...
class xModule(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.features = nn.Sequential(
            ...
            xUnitD(num_features=out_channels, batch_norm=False),
        )
...
class Generator(nn.Module):
    def __init__(self, in_channels, num_features, gen_blocks, dis_blocks, growth_rate, bn_size, scale):
        ...
        for _ in range(int(log(scale, 2))):
            block = UpsampleX2(in_channels=self.num_features, out_channels=self.num_features)
            blocks.append(block)
        self.usample = nn.Sequential(*blocks)

        # features to image
        self.hrconv = nn.Conv2d(in_channels=self.num_features, out_channels=self.num_features, kernel_size=3, padding=1)
...
def g_xdense(**config):
    ...
    config.setdefault('growth_rate', 8)
